;===========================================================================================!
PRO Calculate_kc_ks_dry,t_accum,ks_max,kc_max $
  ,PET_JH,kc_JH,kc_PM,kc_buck,ks_JH,ks_PM,ks_buck,n_plots,fws_JH,fws_PM,fws_buck
  ;-----------------------------------------------------------------------------------------!
  ...
  ;-----------------------------------------------------------------------------------------!
  ; field01 - Jday
  ; field02 - Gregorian date
  ; field03 - mean daily air temperature at 2-m (C) {mostly calculated from max and min temp}
  ; field04 - precipitation (mm d-1)
  ; field05 - solar radiation (MJ d-1)
  ;-----------------------------------------------------------------------------------------!
  ;  Other possible fields.
  ;-----------------------------------------------------------------------------------------!
  ;  field06 - irrigation (mm d-1).
  ;  field07 - mean relative humidity (%).
  ;  field08 - maximum relative humidity (%).
  ;  field09 - minimum relative humidity (%).
  ;  field10 - maximum daily temperature (C).
  ;  field11 - minimum daily temperature (C).
  ;  field12 - total water supply for dry (D) fields (D. Bonfil's plots) (mm d-1).
  ;  field13 - total water supply for wet (W) fields (D. Bonfil's plots) (mm d-1).
  ;  field16 - daily potential ET (PET) calculated from P-M (mm d-1).
  ;  field17 - daily potential ET (PET) estimated from a bucket (mm d-1).
  ;  field18 - daily potential ET (PET) calculated from J-H (mm d-1).
  ;-----------------------------------------------------------------------------------------!
  ...
  WS_D        = Result.field12      ; water supply (precip + irrigation).
  ...
  ;
    ...
    ;-----------------------------------------------------------------------!
        ts_NDVI(p,0:ind-1) = plots_ts_intpol(p,strt_d) ; fill with minimum.
