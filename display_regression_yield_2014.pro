...
;    cult_swc(i,*)  = cul4
  ...
  cgPlot, x_data, y_data, XTitle='swc (mm)' $
    , YTitle='Grain yield (ton/ha)' $
    , SymColor='blue', PSym='Open Circle', symsize=1.25 $
    , Position=position2, charsize=1.25 $
    , yrange=[min(y_data)-0.5,  max(y_data) +  0.5] $
    , xrange=[min(x_data)-20.0, max(x_data) + 20.0]
  ;-----------------------------------------------------------------------!
  ...
  cgText, 0.315, 0.86, /NORMAL, 'y$\up$  = '+ inter $
    + ' + ' + slope +'x', color='blue', charsize=1.25
  cgText, 0.315, 0.80, /NORMAL, 'R$\up2$ = '+rsq_annot $
    , color='blue', charsize=1.25
